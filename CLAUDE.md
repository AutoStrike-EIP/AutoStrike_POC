...
294 techniques are generated by running `make import-mitre` (MITRE ATT&CK STIX + Atomic Red Team inner join, across 12 YAML files).
...
After import: 220 safe, 74 unsafe. Safety is determined **per-executor** based on: `!elevation_required && !hasDangerousPattern(command) && !hasDangerousPattern(cleanup)`. Use `make import-mitre-safe` to import only safe techniques.
